  ...
  if (supabaseAdmin) {
    ...
    const cmRes = await supabaseAdmin
      ...
      .in('course_assignment_id',
        (await supabaseAdmin.from('course_assignment').select('id').eq('batch_id', batchId)).data?.map((r: { id: string }) => r.id) || []
      );
    ...
    type SingleCA = { course_id: string; instructor_id: string };
    type ClassMeetingJoinRow = {
      id: string;
      weekday: number;
      starts_at: string;
      ends_at: string;
      room: string | null;
      course_assignment: SingleCA | SingleCA[] | null;
    };
    const items = ((cmRes.data ?? []) as unknown as ClassMeetingJoinRow[]).map((r) => {
      const ca = Array.isArray(r.course_assignment) ? r.course_assignment[0] : r.course_assignment;
      return {
        ...
        instructor_id: ca?.instructor_id,
      };
    });
